#!/usr/bin/env python3
"""
Builder for Function_Calling.ipynb — generuje notebook identyczny z Function_Calling_final2.

Użycie:
    python _build_fc_notebook.py
    python _build_fc_notebook.py output.ipynb
"""
...
def md_collapsed(source):
    """Markdown cell with jp-MarkdownHeadingCollapsed."""
    return {"cell_type": "markdown", "metadata": {"jp-MarkdownHeadingCollapsed": True}, "source": _split(source)}

def code(source):
    """Code cell."""
    return {"cell_type": "code", "execution_count": None, "metadata": {}, "outputs": [], "source": _split(source)}
...
def student_stub(source):
    ...
    if cell["metadata"]["student_stub_template"][-1] == "\n":
        cell["metadata"]["student_stub_template"][-1] = ""
    return cell

def _split(source):
    """Split source into JSON-compatible line list."""
    slines = source.split("\n")
    return [line + "\n" for line in slines[:-1]] + [slines[-1]]
...
# Cell 0: # Function Calling — LLM jako "mózg" który wywołuje funkcje
cells.append(md("""# Function Calling — LLM jako "mózg" który wywołuje funkcje
...
# Cell 1: ## 1. Konfiguracja środowiska
cells.append(md("""## 1. Konfiguracja środowiska"""))

# Cell 2: from utils import ensure_package
cells.append(code("""from utils import ensure_package
...
# Cell 3: ## 2. Połączenie z LLM-em
cells.append(md("""## 2. Połączenie z LLM-em"""))

# Cell 4: Poniższa komórka automatycznie szuka działającego LLM-a. Zanim ją uruc
cells.append(md("""Poniższa komórka automatycznie szuka działającego LLM-a. Zanim ją uruchomisz:
...
**Na zajęciach pracujemy na:** `gemma4:e4b` — szybki, lekki, świetny w function calling. Pobierz go w LM Studio!

W domu możesz eksperymentować z większymi modelami: `qwen3.6`, `qwen3.5:9b+`, `gemma4:12b`"""))

# Cell 5: from utils import connect_llm, extract_reasoning, print_reasoning, cle
cells.append(code("""from utils import connect_llm, extract_reasoning, print_reasoning, clean_content

# Jeśli nie masz lokalnego LLM-a, wpisz adres serwera prowadzącego (podany na zajęciach):
LECTURER_SERVER = "http://ADRES_SERWERA:4242"  # ← prowadzący poda na zajęciach
...
# Cell 6: # ── System prompt — wspólny dla wszystkich funkcji ──────────────────
cells.append(code("""# ── System prompt — wspólny dla wszystkich funkcji ──────────────────
...
# Cell 7: ## 3. Kalkulator — twoje pierwsze narzędzie + Function Call!
cells.append(md("""## 3. Kalkulator — twoje pierwsze narzędzie + Function Call!"""))

# Cell 8: Zacznijmy od **najprostszego** możliwego narzędzia — kalkulatora.
cells.append(md("""Zacznijmy od **najprostszego** możliwego narzędzia — kalkulatora.
Zobaczymy **cały cykl Function Calling** krok po kroku."""))

# Cell 9: def calculate(expression: str) -> str:
cells.append(code("""def calculate(expression: str) -> str:
    \"\"\"
...
# Cell 10: ### Pierwszy Function Call — pod maską!
cells.append(md("""### Pierwszy Function Call — pod maską!"""))

# Cell 11: Mamy narzędzie. Teraz **opiszemy** je dla LLM-a (w formacie JSON Schem
cells.append(md("""Mamy narzędzie. Teraz **opiszemy** je dla LLM-a (w formacie JSON Schema)
i wyślemy pytanie. LLM **sam zdecyduje** czy potrzebuje kalkulatora.

Zobaczmy **dokładnie** co się dzieje na każdym etapie:"""))

# Cell 12: # Opis narzędzia dla LLM-a (JSON Schema):
cells.append(code("""# Opis narzędzia dla LLM-a (JSON Schema):
calc_tool = {
    "type": "function",
    "function": {
        "name": "calculate",
...
# Cell 13: <div style="background:#d4edda; border-left:4px solid #28a745; padding
cells.append(md("""<div style="background:#d4edda; border-left:4px solid #28a745; padding:14px; border-radius:4px;">
...
# Cell 14: # Przypomnijmy — ręczny JSON Schema naszego kalkulatora:
cells.append(code("""# Przypomnijmy — ręczny JSON Schema naszego kalkulatora:
...
# Cell 15: # Helper — generuje definicję narzędzia z modelu Pydantic.
cells.append(code("""# Helper — generuje definicję narzędzia z modelu Pydantic.
...
# Cell 16: <div style="background:#fff3cd; border-left:4px solid #ffc107; padding
cells.append(md("""<div style="background:#fff3cd; border-left:4px solid #ffc107; padding:12px; border-radius:4px;">
...
# Cell 17: ### 3b. *(opcjonalne)* Instructor — Structured Output
cells.append(md_collapsed("""### 3b. *(opcjonalne)* Instructor — Structured Output"""))

# Cell 18: <div style="background:#e8f4f8; border-left:4px solid #2196F3; padding
cells.append(md("""<div style="background:#e8f4f8; border-left:4px solid #2196F3; padding:12px; border-radius:4px;">
...
# Cell 19: # Prosty model — informacja o mieście
cells.append(code("""# Prosty model — informacja o mieście
...
# Cell 20: # ↓ To NIE jest tekst — to obiekt Pythona! Mamy dostęp do pól: ↓
cells.append(code("""# ↓ To NIE jest tekst — to obiekt Pythona! Mamy dostęp do pól: ↓
if instructor_client:
...
# Cell 21: #### Co się stanie gdy pytanie nie pasuje do modelu?
cells.append(md("""#### Co się stanie gdy pytanie nie pasuje do modelu?"""))

# Cell 22: LLM **musi** wypełnić wszystkie pola — nawet gdy pytanie dotyczy czego
cells.append(md("""LLM **musi** wypełnić wszystkie pola — nawet gdy pytanie dotyczy czegoś zupełnie innego.
Zobaczmy co się stanie gdy zapytamy o smak lodów, a model wymaga danych o mieście:"""))

# Cell 23: # MaybeCityInfo — model z polem "found" na wypadek pytań nie w temacie
cells.append(code("""# MaybeCityInfo — model z polem "found" na wypadek pytań nie w temacie
...
# Cell 24: #### Mechanizm retry — instructor sam naprawia błędy
cells.append(md("""#### Mechanizm retry — instructor sam naprawia błędy"""))

# Cell 25: Jedną z największych zalet `instructor` jest **automatyczny retry z fe
cells.append(md("""Jedną z największych zalet `instructor` jest **automatyczny retry z feedbackiem**.
...
# Cell 26: # --- Sabotaż! Przechwytujemy odpowiedź LLM-a i psujemy JSON ---
cells.append(code("""# --- Sabotaż! Przechwytujemy odpowiedź LLM-a i psujemy JSON ---
...
try:
    ...
    print(f"  ⚠️ Demo się nie powiodło po {_call_count[0]} próbach: {e}")
    print("  (To nie przeszkadza — idziemy dalej!)")
finally:
    # Przywracamy oryginalny client.create (ważne dla dalszych komórek!)
    client.chat.completions.create = _orig_create"""))

# Cell 27: <div style="background:#d4edda; border-left:4px solid #28a745; padding
cells.append(md("""<div style="background:#d4edda; border-left:4px solid #28a745; padding:12px; border-radius:4px;">
...
# Cell 28: #### Function Calling vs. Structured Output — dwa różne mechanizmy!
cells.append(md("""#### Function Calling vs. Structured Output — dwa różne mechanizmy!"""))

# Cell 29: <div style="background:#fff3cd; border-left:4px solid #ffc107; padding
cells.append(md("""<div style="background:#fff3cd; border-left:4px solid #ffc107; padding:14px; border-radius:4px;">
...
# Cell 30: ## 4. Pogoda — prawdziwe API + fallback
cells.append(md("""## 4. Pogoda — prawdziwe API + fallback"""))

# Cell 31: Kalkulator pokazał nam cały cykl FC. Teraz zbudujemy **poważniejsze na
cells.append(md("""Kalkulator pokazał nam cały cykl FC. Teraz zbudujemy **poważniejsze narzędzie**
...
# Cell 32: MOCK_WEATHER = {
cells.append(code("""MOCK_WEATHER = {
...
# Cell 33: ## 5. Baza prezydentów Polski
cells.append(md("""## 5. Baza prezydentów Polski

Trzecie narzędzie — baza danych o prezydentach III RP wczytywana z pliku `prezydenci_polski.md`."""))

# Cell 34: def load_presidents():
cells.append(code("""def load_presidents():
...
# Cell 35: ## 6. Trzy narzędzia — LLM wybiera!
cells.append(md("""## 6. Trzy narzędzia — LLM wybiera!
...
# Cell 36: # Modele Pydantic dla argumentów każdego narzędzia
cells.append(code("""# Modele Pydantic dla argumentów każdego narzędzia
...
# Cell 37: # ── Model ToolReasoning ─────────────────────────────────────────────
cells.append(code("""# ── Model ToolReasoning ──────────────────────────────────────────────
...
# Cell 39: ### Ćwiczenie 1: Co ważniejsze — nazwa czy opis narzędzia?
cells.append(md("""### Ćwiczenie 1: Co ważniejsze — nazwa czy opis narzędzia?
...
# Cell 40: # Ćwiczenie 1A: Zmień description get_weather na coś mylącego
cells.append(student_stub("""# Ćwiczenie 1A: Zmień description get_weather na coś mylącego
import copy
...
# Cell 41: ###### <span style="color: #c17f24;">Podpowiedź</span> <span style="co
cells.append(md_collapsed("""###### <span style="color: #c17f24;">Podpowiedź</span> <span style="color: #999; font-weight: normal; font-size: 0.85em;">(kliknij aby rozwinąć)</span>"""))

# Cell 42: Wpisz np.: `test_tools[0]["function"]["description"] = "Przeszukuje ba
cells.append(md("""Wpisz np.: `test_tools[0]["function"]["description"] = "Przeszukuje bazę danych o prezydentach Polski"`"""))

# Cell 43: ###### <span style="color: #5a8a6a;">Rozwiązanie</span> <span style="c
cells.append(md_collapsed("""###### <span style="color: #5a8a6a;">Rozwiązanie</span> <span style="color: #999; font-weight: normal; font-size: 0.85em;">(kliknij aby rozwinąć)</span>"""))

# Cell 44: # Ćwiczenie 1A — rozwiązanie
cells.append(code("""# Ćwiczenie 1A — rozwiązanie
import copy
...
# Cell 46: # --- Test ćwiczenia 1A (uruchom po uzupełnieniu komórki powyżej) ---
cells.append(code("""# --- Test ćwiczenia 1A (uruchom po uzupełnieniu komórki powyżej) ---
def test_1a(pytanie):
...
# Cell 48: <div style="background:#fff3cd; border-left:4px solid #ffc107; padding
cells.append(md("""<div style="background:#fff3cd; border-left:4px solid #ffc107; padding:14px; border-radius:4px;">
...
# Cell 49: ### Ćwiczenie 1 — Część B: Cichy błąd (najgroźniejszy rodzaj!)
cells.append(md("""### Ćwiczenie 1 — Część B: Cichy błąd (najgroźniejszy rodzaj!)
...
# Cell 50: # Ćwiczenie 1B (demo): Cichy błąd — dwie funkcje z tym samym parametre
cells.append(code("""# Ćwiczenie 1B (demo): Cichy błąd — dwie funkcje z tym samym parametrem + zamienione opisy
...
# Cell 51: # Ćwiczenie 1B (test): Wysyłamy pytanie — LLM widzi podmienione opisy
cells.append(code("""# Ćwiczenie 1B (test): Wysyłamy pytanie — LLM widzi podmienione opisy
...
# Cell 52: <div style="background:#f8d7da; border-left:4px solid #dc3545; padding
cells.append(md("""<div style="background:#f8d7da; border-left:4px solid #dc3545; padding:14px; border-radius:4px;">
...
# Cell 54: ### Ćwiczenie 2: Dodaj własne narzędzie
cells.append(md("""### Ćwiczenie 2: Dodaj własne narzędzie
...
# Cell 55: # Ćwiczenie 2: Dodaj narzędzie get_population
cells.append(code("""# Ćwiczenie 2: Dodaj narzędzie get_population

def get_population(city: str) -> str:
    # Słownik: {"Warszawa": (1_860_000, 1), "Kraków": (800_000, 2), ...}
...
if _test_result is None:
    ...
    print("\\033[1;31m⬆️ Uzupełnij opis narzędzia powyżej!\\033[0m")
else:
...
# Cell 56: ###### <span style="color: #c17f24;">Podpowiedź</span> <span style="co
cells.append(md_collapsed("""###### <span style="color: #c17f24;">Podpowiedź</span> <span style="color: #999; font-weight: normal; font-size: 0.85em;">(kliknij aby rozwinąć)</span>"""))

# Cell 57: Wzoruj się na `get_weather` — słownik z danymi, f-string dla wyniku. D
cells.append(md("""Wzoruj się na `get_weather` — słownik z danymi, f-string dla wyniku. Do `tools_definition` użyj `make_tool()` — potrzebujesz model Pydantic dla **argumentów** (tu: `city: str`) + `tools_definition.append(make_tool("get_population", "...", GetPopulationArgs))`."""))

# Cell 58: ###### <span style="color: #5a8a6a;">Rozwiązanie</span> <span style="c
cells.append(md_collapsed("""###### <span style="color: #5a8a6a;">Rozwiązanie</span> <span style="color: #999; font-weight: normal; font-size: 0.85em;">(kliknij aby rozwinąć)</span>"""))

# Cell 59: # Ćwiczenie 2 — rozwiązanie
cells.append(code("""# Ćwiczenie 2 — rozwiązanie
...
# Cell 62: ## 7. Wikipedia jako narzędzie
cells.append(md("""## 7. Wikipedia jako narzędzie

Pora na **prawdziwe** narzędzie sięgające do internetu!
Biblioteka `wikipedia` pozwala przeszukiwać Wikipedię programatycznie."""))

# Cell 63: ### Ćwiczenie 3: Zbuduj narzędzie Wikipedia
cells.append(md("""### Ćwiczenie 3: Zbuduj narzędzie Wikipedia
...
# Cell 64: import wikipedia
cells.append(student_stub("""import wikipedia
...
if _test_result is None or _test_result is ...:
    ...
    print("\\033[1;31m⬆️ Uzupełnij opis narzędzia powyżej!\\033[0m")
elif SearchWikipediaArgs.model_fields["query"].description == ...:
    print("\\033[1;31m⬆️ Uzupełnij opis argumentów podawanych do narzędzia powyżej!\\033[0m")
else:
    # Usuwamy stary wpis (bezpieczne przy wielokrotnym uruchamianiu)
...
# Cell 65: ###### <span style="color: #c17f24;">Podpowiedź</span> <span style="co
cells.append(md_collapsed("""###### <span style="color: #c17f24;">Podpowiedź</span> <span style="color: #999; font-weight: normal; font-size: 0.85em;">(kliknij aby rozwinąć)</span>"""))

# Cell 66: Funkcja: `wikipedia.search()` zwraca listę tytułów, `wikipedia.page()`
cells.append(md("""Funkcja: `wikipedia.search()` zwraca listę tytułów, `wikipedia.page()` zwraca stronę z polami `.title`, `.url`, `.summary`. Uważaj na `DisambiguationError` — złap go w `try/except`. Narzędzie zwraca zwykły tekst (f-string z tytułem, streszczeniem i URL). Do rejestracji: model Pydantic dla **argumentów** + `make_tool()`."""))

# Cell 67: ###### <span style="color: #5a8a6a;">Rozwiązanie</span> <span style="c
cells.append(md_collapsed("""###### <span style="color: #5a8a6a;">Rozwiązanie</span> <span style="color: #999; font-weight: normal; font-size: 0.85em;">(kliknij aby rozwinąć)</span>"""))

# Cell 68: # Ćwiczenie 3 — rozwiązanie
cells.append(code("""# Ćwiczenie 3 — rozwiązanie
import wikipedia
wikipedia.set_lang("pl")
...
# Cell 71: ## 8. Web search — DuckDuckGo
cells.append(md("""## 8. Web search — DuckDuckGo
...
# Cell 72: from ddgs import DDGS
cells.append(code("""from ddgs import DDGS
...
# Cell 73: Sprawdźmy, czy LLM wybierze `search_web` zamiast `search_wikipedia` — 
cells.append(md("""Sprawdźmy, czy LLM wybierze `search_web` zamiast `search_wikipedia` — pytamy o coś **aktualnego i konkretnego**:"""))

# Cell 74: if client:
cells.append(code("""if client:
...
# Cell 75: ## 8b. Tajemnica w danych — zagrożenie i szansa
cells.append(md("""## 8b. Tajemnica w danych — zagrożenie i szansa
...
# Cell 77: <div style="background:#f8d7da; border-left:4px solid #dc3545; padding
cells.append(md("""<div style="background:#f8d7da; border-left:4px solid #dc3545; padding:14px; border-radius:4px;">
...
# Cell 78: ## 9. Combo: Function Calling + Structured Output
cells.append(md("""## 9. Combo: Function Calling + Structured Output
...
# Cell 79: def verify_claim(claim: str) -> "FactCheck":
cells.append(code("""def verify_claim(claim: str) -> "FactCheck":
    \"\"\"
    Combo pattern:
...
# Cell 80: ### Ćwiczenie 4: FactCheck — zweryfikuj twierdzenie strukturalnie
cells.append(md("""### Ćwiczenie 4: FactCheck — zweryfikuj twierdzenie strukturalnie
...
# Cell 81: # Ćwiczenie 4: Uzupełnij model FactCheck
cells.append(student_stub("""# Ćwiczenie 4: Uzupełnij model FactCheck

class FactCheck(BaseModel):

    pass  # ✏️ Tutaj wpisz 5 pól opisanych powyżej

if len(FactCheck.model_fields) == 0:
    print("\\033[1;31m⬆️ Uzupełnij pola w FactCheck!\\033[0m")"""))

# Cell 82: ###### <span style="color: #c17f24;">Podpowiedź</span> <span style="co
cells.append(md_collapsed("""###### <span style="color: #c17f24;">Podpowiedź</span> <span style="color: #999; font-weight: normal; font-size: 0.85em;">(kliknij aby rozwinąć)</span>"""))

# Cell 83: Użyj `Literal["prawda", "fałsz", "nie da się zweryfikować"]` dla verdi
cells.append(md("""Użyj `Literal["prawda", "fałsz", "nie da się zweryfikować"]` dla verdict, `Field(..., ge=0, le=1)` dla confidence. Każde pole potrzebuje `Field(..., description="...")`. Funkcja `verify_claim()` jest gotowa — wystarczy uzupełnić model."""))

# Cell 84: ###### <span style="color: #5a8a6a;">Rozwiązanie</span> <span style="c
cells.append(md_collapsed("""###### <span style="color: #5a8a6a;">Rozwiązanie</span> <span style="color: #999; font-weight: normal; font-size: 0.85em;">(kliknij aby rozwinąć)</span>"""))

# Cell 85: # Ćwiczenie 4 — rozwiązanie (tylko model — verify_claim jest gotowa po
cells.append(code("""# Ćwiczenie 4 — rozwiązanie (tylko model — verify_claim jest gotowa powyżej)
...
# Cell 88: <div style="background:#e8f4f8; border-left:4px solid #2196F3; padding
cells.append(md("""<div style="background:#e8f4f8; border-left:4px solid #2196F3; padding:14px; border-radius:4px;">
...
# Cell 90: ## 10. Pętla agentowa — LLM który myśli i działa
cells.append(md("""## 10. Pętla agentowa — LLM który myśli i działa
...
# Cell 91: def agent(question, max_steps=6):
cells.append(code("""def agent(question, max_steps=6):
    if not client:
        print("LLM niedostępny.")
        return
...
# Cell 93: ### Ćwiczenie 5: Zadaj agentowi trudne pytanie
cells.append(md("""### Ćwiczenie 5: Zadaj agentowi trudne pytanie
...
# Cell 94: # Ćwiczenie 5: Twoje pytanie do agenta
cells.append(student_stub("""# Ćwiczenie 5: Twoje pytanie do agenta
...
# Cell 95: ###### <span style="color: #c17f24;">Podpowiedź</span> <span style="co
cells.append(md_collapsed("""###### <span style="color: #c17f24;">Podpowiedź</span> <span style="color: #999; font-weight: normal; font-size: 0.85em;">(kliknij aby rozwinąć)</span>"""))

# Cell 96: Przykłady pytań łączących narzędzia:
cells.append(md("""Przykłady pytań łączących narzędzia:
...
# Cell 98: ### Ćwiczenie 6 (bonus): Zbuduj własnego asystenta
cells.append(md("""### Ćwiczenie 6 (bonus): Zbuduj własnego asystenta
...
# Cell 99: # Ćwiczenie 6 (bonus): Twój asystent
cells.append(student_stub("""# Ćwiczenie 6 (bonus): Twój asystent
...
# Cell 100: ###### <span style="color: #c17f24;">Podpowiedź</span> <span style="co
cells.append(md_collapsed("""###### <span style="color: #c17f24;">Podpowiedź</span> <span style="color: #999; font-weight: normal; font-size: 0.85em;">(kliknij aby rozwinąć)</span>"""))

# Cell 101: Dla asystenta podróżniczego: `SYSTEM_PROMPT = "Jesteś asystentem podró
cells.append(md("""Dla asystenta podróżniczego: `SYSTEM_PROMPT = "Jesteś asystentem podróżniczym. Pomagasz planować wycieczki. Używaj Wikipedii do znalezienia informacji o miastach, pogody do sprawdzenia warunków, i kalkulatora do przeliczeń walutowych. Odpowiadaj po polsku, entuzjastycznie."`"""))

# Cell 102: ######
cells.append(md("""###### """))

# Cell 103: ## 11. Podsumowanie
cells.append(md("""## 11. Podsumowanie"""))

# Cell 104: ### Co zrobiliśmy?
cells.append(md("""### Co zrobiliśmy?
...
# Cell 105: ## 12. Bonus: Chat z pamięcią konwersacji
cells.append(md("""## 12. Bonus: Chat z pamięcią konwersacji
...
# Cell 106: def chat_with_tools(messages, question, verbose=True):
cells.append(code("""def chat_with_tools(messages, question, verbose=True):
...
# Cell 107: # Testujemy! Każde kolejne pytanie pamięta kontekst poprzednich.
cells.append(code("""# Testujemy! Każde kolejne pytanie pamięta kontekst poprzednich.
...
# Cell 108: ## 13. 🚀 Bonus: Chat UI — jak ChatGPT, ale Twój!
cells.append(md("""## 13. 🚀 Bonus: Chat UI — jak ChatGPT, ale Twój!
...
# Cell 109: from utils import ensure_package
cells.append(code("""from utils import ensure_package
...
# Cell 110: <div style="background:#e8f4f8; border-left:4px solid #17a2b8; padding
cells.append(md("""<div style="background:#e8f4f8; border-left:4px solid #17a2b8; padding:14px; border-radius:4px;">
